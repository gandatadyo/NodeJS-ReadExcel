...
exports.OpenQueryParam = (ssql, parameter, resolve) => {
    setTimeout(() => {
        // connect mysql to query
        let pool = app.module_app();
        pool.getConnection(function (err, connection) {
            if (err) throw err;
            connection.query(ssql, parameter, function (error, results, fields) {
                connection.release();
                if (error) throw error;
                sdatareturn = {
                    error: error,
...
exports.Sessionaccount = (req) => {
    let sstatus = '';
    if (!req.session.username) sstatus = 'false';
    else sstatus = 'true'
    let stemp = {
        'idusername': req.session.idusername,
        'username': req.session.username,
        'status': sstatus // type string
    }
    return stemp;
}
...
exports.ExecQueryParam = (ssql, parameter, resolve) => {
    setTimeout(() => {
        // connect mysql to query
        let pool = app.module_app();
        pool.getConnection(function (err, connection) {
            if (err) throw err;
            connection.query(ssql, parameter, function (err, result) {
                connection.release();
                if (err) throw err;
                sdatareturn = {
                    error: err,
...
exports.UploadFile = (dataFile, uploadPath, resolve) => {
...
exports.GetSQLQuery = async (NameQuery, resolve) => {
    try {
        let ssql_Resolve = await new Promise((resolve) => { router_module.OpenQuery(`SELECT DataQuery FROM dbsquery WHERE NameQuery="${NameQuery}"`, resolve) })
        resolve(ssql_Resolve.dataset[0].DataQuery)
    } catch (error) {
        console.log(error);
    }
}
...
exports.GetDataCategory = async (resolve) => {
    try {
        let dataCategory_Resolve = await new Promise((resolve) => { router_module.OpenQuery("SELECT NameCategory FROM `dbmcategory` WHERE isWebsite='1'", resolve) })
        resolve(dataCategory_Resolve.dataset)
    } catch (error) {
        console.log(error);
    }
